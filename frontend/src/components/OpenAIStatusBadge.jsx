...
export default function OpenAIStatusBadge() {
  ...
  const classify = (payload) => {
    if (!payload) return { label: "Błąd połączenia", tone: "error" };
    if (payload.ok === true) return { label: "OpenAI: OK", tone: "ok" };
    if (payload.error === "missing_key") return { label: "OpenAI: brak klucza", tone: "warn" };
    if (payload.status === 401 || /incorrect api key/i.test(payload?.body || "")) return { label: "OpenAI: zły klucz", tone: "error" };
  ...
  const pull = async () => {
    try {
      ...
      if (lastToneRef.current !== c.tone) lastToneRef.current = c.tone;
      ...
      <dialog ref={dlgRef} className="rounded-md p-0 w-[min(90vw,640px)]">
        <form method="dialog">
          <div className="p-4 border-b">
            ...
            <div className="text-sm text-zinc-500">Pole jest <b>lokalne</b> – tylko do wygenerowania komend. Nie wysyłamy klucza.</div>
          </div>
          <div className="p-4 space-y-3">
            <div>
              <label className="text-sm">Klucz OpenAI</label>
              <input type="text" placeholder="sk-..." onChange={(e)=>keyRef.current=e.target.value}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button type="button" className="px-3 py-1 border rounded"
                      onClick={()=>copy(`OPENAI_API_KEY=${keyRef.current}`)}>
                Kopiuj dla panelu env
              </button>
              <button type="button" className="px-3 py-1 border rounded"
                      onClick={()=>copy(`curl -s http://localhost/api/openai/status | jq .`)}>
                Kopiuj curl
              </button>
              <button type="button" className="px-3 py-1 border rounded"
                      onClick={()=>{ nextDelayRef.current=5000; pull(); }}>
                Testuj teraz
              </button>
            </div>
            <details className="text-xs">
              <summary className="cursor-pointer">Pokaż surowy wynik</summary>
              <pre className="mt-2 p-2 bg-zinc-100 dark:bg-zinc-800 rounded overflow-auto">
{state.raw ? JSON.stringify(state.raw, null, 2) : "(brak danych)"}
              </pre>
            </details>
            <div className="text-xs text-zinc-500">
              🔐 Klucz trzymaj w env backendu / sekrecie CI. 🔁 Po ustawieniu zrestartuj backend.
            </div>
          </div>
